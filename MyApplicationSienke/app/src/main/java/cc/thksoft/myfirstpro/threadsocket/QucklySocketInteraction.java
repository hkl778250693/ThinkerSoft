package cc.thksoft.myfirstpro.threadsocket;
...
import cc.thksoft.myfirstpro.util.SocketConnection;
import cc.thksoft.myfirstpro.util.UniqueID;

public class QucklySocketInteraction {
	...
	public QucklySocketInteraction(Context context,int port,String ip,String operName,String parameter,Handler dataHandler){
		this.ip = ip;
	...
	public boolean DataDownLoadConn(){
		...
		try{
			...
			if(signel == true){
				...
				downresult = socketConn.QucklyDownLoadData(bufferSize).trim();
				...
				if(downresult.contains("B")||"".equals(downresult)){
					...
					for(int i=0;i<bagCount;i++){
						...
						socketConn.UpLoadData("A");
						downresult = socketConn.QucklyDownLoadData(tempsize).trim();
						if("".equals(downresult))
							return false;
			...
			if(signel == true){
				...
				if(downresult.contains("B"))
				{
					...
					for(int i=0;i<bagCount;i++)
					{
						...
						if(parameter.contains("28��")||parameter.contains("29��")||parameter.contains("30��")){////29 �ɼ�GPS���ꡢ30 �����豸
					...
					//{"status":1,"error":"�ɼ��û�����ɹ�"}

	/*				if("".equals(downresult)||(downresult.contains("2")&&downresult.contains("error"))){
						return false;
					}*/
					downresult = socketConn.QucklyDownLoadData(bufferSize).trim();//C
	...
	private String analyzeDeviceData(String Result){
		...
		if(map!=null&&!map.isEmpty()){
			if("1".equals(map.get("status"))){//null��ɼ��û�����ɹ�
