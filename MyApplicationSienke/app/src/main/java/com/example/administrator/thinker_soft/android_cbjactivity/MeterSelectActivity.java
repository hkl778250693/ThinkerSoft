...
public class MeterSelectActivity extends Activity {
    ...
    public void getAreaData() {
        areaInfoList.clear();
        areaCursor = db.rawQuery("select * from MeterArea where userId=?", new String[]{sharedPreferences_login.getString("userId","")});//查询并获得游标
        //如果游标为空，则显示没有数据图片
    ...
    public void getbookData() {
        bookInfoList.clear();
        bookCursor = db.rawQuery("select * from MeterBook where userId=?", new String[]{sharedPreferences_login.getString("userId","")});//查询并获得游标
        //如果游标为空，则显示没有数据图片
