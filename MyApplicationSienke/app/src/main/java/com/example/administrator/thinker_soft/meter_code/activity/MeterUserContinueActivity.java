...
public class MeterUserContinueActivity extends Activity {
    ...
    private void defaultSetting() {
        ...
        new Thread() {
            ...
            public void run() {
                super.run();
                getMeterUserData(dataStartCount);//读取本地的抄表分区用户数据
                getContinueMeterPosition();
    ...
    //读取本地的抄表分区用户数据
    public void getMeterUserData(int dataStartCount) {
        userLists.clear();
    ...
    //获取继续抄表的item位置
    public void getContinueMeterPosition() {
        Log.i("ContinueCheckPosition", "获取继续安检位置进来了！");
        Cursor cursor = db.rawQuery("select * from MeterUser where login_user_id=?", new String[]{sharedPreferences_login.getString("userId", "")});//查询并获得游标
