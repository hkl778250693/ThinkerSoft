 ...
 *******************************************************************************/
package com.example.administrator.thinker_soft.myfirstpro.refreshListView;
...
public abstract class RefreshPullToRefreshBase<T extends View> extends LinearLayout
    implements
      RefreshIPullToRefresh<T> {
  ...
  private RefreshLoadingLayout mHeaderLayout;

  private RefreshLoadingLayout mFooterLayout;
  ...
  private boolean topsignal;
  
  private boolean bottomsignal;
  ...
  public RefreshPullToRefreshBase(Context context) {
    super(context);
    init(context, null);
  }

  public RefreshPullToRefreshBase(Context context, AttributeSet attrs) {
    super(context, attrs);
    init(context, attrs);
  }

  public RefreshPullToRefreshBase(Context context, Mode mode) {
    super(context);
    mMode = mode;
    init(context, null);
  }
  ...
  public final boolean isPullToRefreshOverScrollEnabled() {
    return VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD && mOverScrollEnabled
        && RefreshOverscrollHelper.isAndroidOverScrollEnabled(mRefreshableView);
  }
  ...
  /**
   * @return Either {@link #VERTICAL_SCROLL} or {@link #HORIZONTAL_SCROLL} depending on the scroll
   *         direction.
   */
  ...
  protected RefreshLoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs,boolean topsignal,boolean bottomsignal) {
    //TODO
	  this.bottomsignal = bottomsignal;
	  this.topsignal = topsignal;
	  return mLoadingAnimationStyle.createLoadingLayout(context, mode,
        getPullToRefreshScrollDirection(), attrs, topsignal, bottomsignal);
  }
   ...
   * class. Be sure to set the ID of the view in this method, especially if you're using a
  ...
  protected final RefreshLoadingLayout getFooterLayout() {
  ...
  protected final RefreshLoadingLayout getHeaderLayout() {
  ...
  /**
   * @deprecated This is a deprecated callback. If you're overriding this method then use
   *             {@link #setOnPullEventListener(OnPullEventListener) setOnPullEventListener()}
   *             instead to be notified when the state changes to {@link State#RESET RESET}. This
   ...
   * @deprecated This is a deprecated callback. If you're overriding this method then use
   *             {@link #setOnPullEventListener(OnPullEventListener) setOnPullEventListener()}
   *             instead to be notified when the state changes to {@link State#REFRESHING
   *             REFRESHING} or {@link State#MANUAL_REFRESHING MANUAL_REFRESHING}. This deprecated
  ...
  private void addRefreshableView(Context context, T refreshableView) {
    ...
    switch (getPullToRefreshScrollDirection()) {
      case HORIZONTAL_SCROLL:
        addViewInternal(mRefreshableViewWrapper, new LayoutParams(0,
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    mLoadingAnimationStyle =
        AnimationStyle.mapIntToValue(a.getInteger(R.styleable.PullToRefresh_ptrAnimationStyle, 1));
    ...
    mHeaderLayout = createLoadingLayout(context, Mode.PULL_FROM_START, a,topsignal,bottomsignal);//TODO
    mFooterLayout = createLoadingLayout(context, Mode.PULL_FROM_END, a,topsignal,bottomsignal);//TODO
   // mFooterLayout.setPullLabel(context.getString(R.string.pulldown_to_loading));
    ...
    if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
      ...
      RefreshUtils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
  ...
  public static enum AnimationStyle {
    ...
    RefreshLoadingLayout createLoadingLayout(Context context, Mode mode, int scrollDirection,
        TypedArray attrs,boolean topsignal,boolean bottomsignal) {
      switch (this) {
        case ROTATE:
        default:
          return new RefreshRotateLoadingLayout(context, mode, scrollDirection, attrs, topsignal, bottomsignal);
        case FLIP:
          return new RefreshFlipLoadingLayout(context, mode, scrollDirection, attrs, topsignal, bottomsignal);
      }
    }
  }
   ...
   * AdapterView. See ( {@link RefreshPullToRefreshAdapterViewBase#setOnLastItemVisibleListener}.
   * 
   * @author Chris Banes
   */
   ...
   * {@link RefreshPullToRefreshAdapterViewBase#setOnPullEventListener}.
   * 
   * @author Chris Banes
   */
  public static interface OnPullEventListener<V extends View> {
     ...
     *        depending on which direction the user is pulling. Only useful when <var>state</var> is
     *        {@link State#PULL_TO_REFRESH} or {@link State#RELEASE_TO_REFRESH}.
     */
    public void onPullEvent(final RefreshPullToRefreshBase<V> refreshView, State state, Mode direction);
  ...
  public static interface OnRefreshListener<V extends View> {
     ...
     */
    public void onRefresh(final RefreshPullToRefreshBase<V> refreshView);
  ...
  public static interface OnRefreshListener2<V extends View> {
     ...
     * released.
     */
    public void onPullDownToRefresh(final RefreshPullToRefreshBase<V> refreshView);

    /**
     * onPullUpToRefresh will be called only when the user has Pulled from the end, and released.
     */
    public void onPullUpToRefresh(final RefreshPullToRefreshBase<V> refreshView);
